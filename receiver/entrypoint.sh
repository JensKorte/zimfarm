...
echo "* *  * * *  root  /usr/bin/flock -w 0 /dev/shm/cron.lock /usr/local/bin/check_zims.sh $ZIM_SRC_DIR $ZIM_DST_DIR $ZIM_QUAR_DIR $VALIDATION_LOG_DIR \"$ZIMCHECK_OPTION\" $VALIDATION_OPTION >> /dev/shm/check_zim.log 2>&1" >> /etc/cron.d/zimvalidation
chmod +x /etc/cron.d/zimvalidation
